...
import static mcp.mobius.waila.addons.ic2.IC2Plugin.CropCard_canBeHarvested;
...
public class HUDHandlerIC2Crop extends DefaultCropProvider implements IDataProvider {
    ...
    public List<String> getGrowthDetails(ItemStack itemStack, IDataAccessor accessor, IPluginConfig config) {
        ...
        try {
            if (TileEntityCrop_id.getInt(te) >= 0) {
                Object card = TileEntityCrop_crop.invoke(te);
                if ((Boolean) CropCard_canBeHarvested.invoke(card, te)) {
                    ret.add(getMatureString(itemStack, accessor, config));
                }
            }
        } catch (Throwable t) {
