...
public class WidgetGeometry {
    ...
    public Point getUnalignedPos(IWidget parent) {
        int x = -1;
        if (this.fracPosX)
            x = (int) Math.ceil(parent.getPos().getX() + parent.getSize().getX() * this.x / 100D);
        ...
        if (this.fracPosY && parent != null)
            y = (int) Math.ceil(parent.getPos().getY() + parent.getSize().getY() * this.y / 100D);
        if (!this.fracPosY && parent != null)
    ...
    public Point getSize(IWidget parent) {
        int sx = -1;
        if (this.fracSizeX)
            sx = (int) Math.ceil(parent.getSize().getX() * this.sx / 100D);
        if (!this.fracSizeX)
            sx = (int) this.sx;

        int sy = -1;
        if (this.fracSizeY)
            sy = (int) Math.ceil(parent.getSize().getY() * this.sy / 100D);
        if (!this.fracSizeY)
            sy = (int) this.sy;
