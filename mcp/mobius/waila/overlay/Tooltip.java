			...
			for (int c = 0; c < lines.get(i).size(); c++){	// We check all the columns for this line
				...
				while (lineMatcher.find()){
					...
					if (renderMatcher.find()){
						...
						if (renderer != null){
							renderable = new Renderable(renderer, new Point(offsetX, offsetY), renderMatcher.group(2).split(","));
							this.elements2nd.add(renderable);
						}
					} else if (iconMatcher.find()){
						renderable = new Renderable(new TTRenderIcon(iconMatcher.group(1)), new Point(offsetX, offsetY));
						this.elements2nd.add(renderable);
					} else {
