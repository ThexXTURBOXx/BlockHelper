    ...
    public static void notifyUpdater(Minecraft mc) {
        if (!isLatestVersion()) {
            if (getLatestVersion().equals(mod_BlockHelper.VERSION)) {
                mc.ingameGUI.addChatMessage(I18n.format("update_check_failed_chat", mod_BlockHelper.NAME));
            } else {
                mc.ingameGUI.addChatMessage(I18n.format("newer_version_available_chat", mod_BlockHelper.NAME,
